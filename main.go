...
func main() {
	...
	engine.GET("/news/issue", func(ctx *gin.Context) { ctx.HTML(http.StatusOK, "news_issue.html", nil) })
	engine.POST("/news/issue/submit", handler.Auth, handler.PostNews)
	engine.GET("/news/belong", handler.NewsBelong)
	engine.GET("/news/:id", handler.GetNewsById)
	engine.DELETE("/news/delete/:id", handler.Auth, handler.DeleteNews)
	engine.POST("/news/update", handler.Auth, handler.UpdateNews)
